...
namespace Camera2Basic
{
    public class Camera2BasicFragment : Fragment, View.IOnClickListener, FragmentCompat.IOnRequestPermissionsResultCallback
    {
        ...
        TextView carId;
        TextView gateStaus;
        static Button in_out_button;
        private static string getStatusUrl = "https://counterfunctions20200429005139.azurewebsites.net/api/get-isOpen";
        private static string _status;
        private static string tempid = "";
        private static string tempInOut = "";
        ...
        public void ShowToast(string text)
        {
            ...
            {
                mut.WaitOne();
                Activity.RunOnUiThread(new ShowToastRunnable(Activity.ApplicationContext, text, carId, gateStaus, mFile));
                mut.ReleaseMutex();
            }
        }
        ...
        private class ShowToastRunnable : Java.Lang.Object, IRunnable
        {
            ...
            private TextView gateStaus;
            private File mFile;
            public ShowToastRunnable(Context context, string text, TextView carId, TextView gateStaus, File mFile)
            {
            ...
            public async void Run()
            {
                ...
                {
                    int n;
                    string s = await getcarplatAsync(mFile.Path);
                    carId.Text = "no number";
                    if (int.TryParse(s, out n))
                    {
                        carId.Text = n.ToString();
                    }
                    if (carId.Text != "no number")
                    {
                        if (carId.Text != tempid || tempInOut != in_out_button.Text)
                        {
                            gateStaus.Text = GetStatus(carId.Text);
                            tempid = carId.Text;
                            tempInOut = in_out_button.Text;
                        }
                    }
                    else
                    {
                        gateStaus.Text = "don't open";
                        tempid = "";
                        tempInOut = "";
                    }
                }
        ...
        {
            if (in_out_button.Text == "in") in_out_button.Text = "out";
            else in_out_button.Text = "in";
        }
        ...
        private async void check_for_car()
        {
            while (true)
            {
                Thread.Sleep(3500);
        ...
        public override void OnViewCreated(View view, Bundle savedInstanceState)
        {
            ...
            carId = (TextView)view.FindViewById(Resource.Id.carId);
            gateStaus = (TextView)view.FindViewById(Resource.Id._status);
            in_out_button = (Button)view.FindViewById(Resource.Id.in_out);
            in_out_button.Click += new EventHandler(clicked);
        }
        ...
        public override void OnActivityCreated(Bundle savedInstanceState)
        {
            ...
            Thread thread = new Thread(check_for_car);
            thread.Start();
        }
        ...
        private async static System.Threading.Tasks.Task<string> getcarplatAsync(string path)
        {
            string base64String = "";
            //byte[] bytes = System.Text.Encoding.ASCII.GetBytes(Convert.ToBase64String(System.IO.File.ReadAllBytes(path)));
            //base64String = bytes.ToString();
            string formDataBoundary = System.String.Format("----------{0:N}", Guid.NewGuid());
            string p = await Program.getCarId(base64String, path, null, "il", formDataBoundary);
            return p;
        }

        public static string GetStatus(string Id)
        {
            string searchApiUrl = $"{getStatusUrl}/{Id}/{Camera2BasicFragment.in_out_button.Text}";
            var results = Camera2BasicFragment.ExecuteWebRequest(searchApiUrl, method: "GET");
            ...
            _status = jsonItemsResults["isOpen"].ToString();
            return _status;
        }
        public static WebResponseTuple ExecuteWebRequest(string requestUrl, string content = "", string contentType = "application/json", string method = "POST")
        {
            ...
            if (content.Length > 0)
            {
                byte[] dataStream = System.Text.Encoding.UTF8.GetBytes(content);
